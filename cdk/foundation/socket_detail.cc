...
Socket connect(const char *host_name, unsigned short port)
{
  ...
  while (connect_result != 0 && host)
  {
    try
    {
      ...
      #endif
        {
          int select_result = poll_one(socket, SELECT_MODE_WRITE, true);
...
Socket listen_and_accept(unsigned short port)
{
  ...
  try
  {
    ...
    int select_result = poll_one(acceptor, SELECT_MODE_READ, true);
...
#ifdef _WIN32
  int result = ::WSAPoll(&fds, 1, wait ? -1 : 0);
#else
  int result = ::poll(&fds, 1,  wait ? -1 : 0);
#endif
...
size_t recv_some(Socket socket, byte *buffer, size_t buffer_size, bool wait)
{
  ...
  */
  ...
  int select_result = poll_one(socket, SELECT_MODE_READ, wait);
...
size_t send_some(Socket socket, const byte *buffer, size_t buffer_size, bool wait)
{
  ...
  */
  ...
  int select_result = poll_one(socket, SELECT_MODE_WRITE, wait);

...
{
public:
     virtual my_ulonglong  mysql_affected_rows      ( MYSQL *       )   _PURE;

     virtual my_bool       mysql_autocommit         ( MYSQL *
                                                    , my_bool       )   _PURE;

     virtual void          mysql_close              ( MYSQL *mysql  )   _PURE;

     virtual my_bool       mysql_commit             ( MYSQL *mysql  )   _PURE;

     virtual void          mysql_data_seek          ( MYSQL_RES *
                                                    , my_ulonglong  )   _PURE;

     virtual void          mysql_debug              ( const char *  )   _PURE;

     virtual unsigned int  mysql_errno              ( MYSQL *mysql  )   _PURE;

     virtual const char *  mysql_error              ( MYSQL *mysql  )   _PURE;

     virtual MYSQL_FIELD * mysql_fetch_field        ( MYSQL_RES *   )   _PURE;

     virtual MYSQL_FIELD * mysql_fetch_field_direct ( MYSQL_RES *
                                                    , unsigned int  )   _PURE;

     virtual unsigned long * mysql_fetch_lengths    ( MYSQL_RES *   )   _PURE;

     virtual MYSQL_ROW     mysql_fetch_row          ( MYSQL_RES *   )   _PURE;

     virtual unsigned int  mysql_field_count        ( MYSQL *       )   _PURE;

     virtual void          mysql_free_result        ( MYSQL_RES *   )   _PURE;

     virtual unsigned long mysql_get_client_version ()                  _PURE;

     virtual const char *  mysql_get_server_info    ( MYSQL *       )   _PURE;

     virtual unsigned long mysql_get_server_version ( MYSQL *       )   _PURE;

     virtual MYSQL *       mysql_init               ( MYSQL *mysql  )   _PURE;

     virtual int           mysql_library_init       ( int   argc
                                                    , char **argv
                                                    , char **groups )   _PURE;

     virtual void          mysql_library_end        ()                  _PURE;

     virtual my_bool       mysql_more_results       ( MYSQL *       )   _PURE;

     virtual int           mysql_next_result        ( MYSQL *       )   _PURE;

     virtual unsigned int  mysql_num_fields         ( MYSQL_RES *   )   _PURE;

     virtual my_ulonglong  mysql_num_rows           ( MYSQL_RES *   )   _PURE;

     virtual int           mysql_options            ( MYSQL *
                                                    , enum mysql_option option
                                                    , const void *arg ) _PURE;

     virtual int		   mysql_query              ( MYSQL *
                                                    , const char *  )   _PURE;

     virtual MYSQL *       mysql_real_connect       ( MYSQL *       mysql
                                                    , const char *  host
                                                    , const char *  user
                                                    , const char *  passwd
                                                    , const char *  db
     ...
     virtual int           mysql_real_query         ( MYSQL *
                                                    , const char *
                                                    , unsigned long )   _PURE;

     virtual my_bool       mysql_rollback           ( MYSQL *       )   _PURE;

     virtual const char *  mysql_sqlstate           ( MYSQL *       )   _PURE;

     virtual my_bool       mysql_ssl_set            ( MYSQL      * mysql
                                                    , const char * key
                                                    , const char * cert
                                                    , const char * ca
                                                    , const char * capath
     ...
     virtual MYSQL_RES *   mysql_store_result       ( MYSQL *       )   _PURE;

     virtual MYSQL_RES *   mysql_use_result         ( MYSQL *       )   _PURE;

     /* Methods - wrappers of prepared statement mysql_stmt_* functions */
     virtual my_ulonglong  mysql_stmt_affected_rows ( MYSQL_STMT *  )   _PURE;

     virtual my_bool       mysql_stmt_attr_set      ( MYSQL_STMT *stmt
                                                    , enum enum_stmt_attr_type option
                                                    , const void *arg ) _PURE;

     virtual my_bool       mysql_stmt_bind_param    ( MYSQL_STMT *
                                                    , MYSQL_BIND *  )   _PURE;

     virtual my_bool       mysql_stmt_bind_result   ( MYSQL_STMT *
                                                    , MYSQL_BIND *  )   _PURE;

     virtual my_bool       mysql_stmt_close         ( MYSQL_STMT *  )   _PURE;

     virtual void          mysql_stmt_data_seek     ( MYSQL_STMT *
                                                    , my_ulonglong  )   _PURE;

     virtual unsigned int  mysql_stmt_errno         ( MYSQL_STMT *  )   _PURE;

     virtual const char *  mysql_stmt_error         ( MYSQL_STMT *  )   _PURE;

     virtual int           mysql_stmt_execute       ( MYSQL_STMT *  )   _PURE;

     virtual int           mysql_stmt_fetch         ( MYSQL_STMT *  )   _PURE;

     virtual unsigned int  mysql_stmt_field_count   ( MYSQL_STMT *  )   _PURE;

     virtual MYSQL_STMT *  mysql_stmt_init          ( MYSQL *       )   _PURE;

     virtual my_ulonglong  mysql_stmt_num_rows      ( MYSQL_STMT *  )   _PURE;

     virtual unsigned long mysql_stmt_param_count   ( MYSQL_STMT *  )   _PURE;

     virtual int           mysql_stmt_prepare       ( MYSQL_STMT *
                                                    , const char *
                                                    , unsigned long )   _PURE;

     virtual MYSQL_RES *   mysql_stmt_result_metadata( MYSQL_STMT * )   _PURE;

     virtual my_bool       mysql_stmt_send_long_data( MYSQL_STMT *  stmt 
                                                    , unsigned int  par_number
                                                    , const char *  data
                                                    , unsigned long len) _PURE;

     virtual const char *  mysql_stmt_sqlstate      ( MYSQL_STMT *  )   _PURE;

     virtual int           mysql_stmt_store_result  ( MYSQL_STMT *  )   _PURE;
};
